...
int main(int argc, char **argv)
{
	...
	clock_t begin = clock();
	// thread_args[0].numbers = numbers;
	// thread_args[0].start = 0;
	// thread_args[0].len = len;
	// thread_function(thread_args);
	...
	printf("L=%ld, t=%ld; Running time is: %f\n",l_end, n_threads, (double)(end-begin)/CLOCKS_PER_SEC);
